...
public class SegmentTree {
    ...
    private int getRangeSumHelper(int leftBound, int rightBound, int queryLeftBound, int queryRightBound, int pos) {
        // Case I : Segment completely inside range
        if (leftBound >= queryLeftBound && rightBound <= queryRightBound) return containerArray[pos];

        // Case II : Segment completely outside range
        if (rightBound < queryLeftBound || leftBound > queryRightBound) return 0;

        // Case III : Overlapping segment
        int mid = leftBound + (rightBound - leftBound) / 2;

        int leftSum = getRangeSumHelper(leftBound, mid, queryLeftBound, queryRightBound, 2 * pos + 1);
        int rightSum = getRangeSumHelper(mid + 1, rightBound, queryLeftBound, queryRightBound, 2 * pos + 2);
